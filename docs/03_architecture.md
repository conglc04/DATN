...
rApp 1s vs xApp 10ms gap → bridge bằng **λ_warm** (phase-aware prior, [05](05_agent_workflow.md)). KHÔNG predict (LSTM đã loại).
...
## Phase FSM
5 pha (φ₁ STANDBY → φ₂ DISPATCH → φ₃ SCENE → φ₄ TRANSPORT → φ₅ RETURN); transition từ **explicit signaling** (C-V2X/E2SM extension hoặc MAC CE), KHÔNG ML inference. Mỗi pha → bộ QoS (D_max, ε, R_min) [02](02_requirements.md).
