...
gNB/cell-center=`(0,0)` (local Cartesian, điểm hội tụ 3 xe trên Giải Phóng), R_cell=1km, UMa 3GPP TR 38.901 single-cell + interference margin −86 dBm/PRB, no handover — KHÔNG đổi giữa các tuần. GPS thật BV Bạch Mai (`config.py: BACH_MAI_LAT/LON`) chỉ dùng cho lớp OSM/SUMO (W15).
...
| **3A** | W18 | test_intra_slice + test_multi_amb pass; assert obs 32/54; PPO K=1 hội tụ; ~261 tests xanh |
| **3B** | W19 | PPO K=3 hội tụ trên obs=54; ≥1 kịch bản structural guarantee (ordering + no-starvation) |
| **3C** | W20 | TD3 K=1 hội tụ; C1–C5 trong ngưỡng hoặc λ phản ứng đúng hướng |
| **3D** | W21 | TD3 K=3 hội tụ trên obs=54; ≥1 kịch bản structural guarantee cho TD3 |
| **3E** | W22 | SAC code mirror TD3 (BaselineFlags/LambdaState); SAC K=1 hội tụ (α hội tụ hợp lý) |
| **3F** | W23 | SAC K=3 hội tụ trên obs=54; Table I (6 cell, 3 solver × K∈{1,3}) + Table II (severity/intra-slice K=3) có CI + p-value hiệu chỉnh |
...
## Critical path
B1 (refs) là cổng cho toàn bộ formulation; B5 (code K≥2 + assert obs 32/54) là cổng cho cả 6 tuần sweep W18–W23; W23 (Table I/II, 3 solver × K∈{1,3}) = headline. SUMO/OSM (B3) là hạ tầng GPS cho mobility (W15), KHÔNG phải eval harness của sweep W18–W23. **E3/E4 → future work**, KHÔNG còn trên critical path.
