...
- Normalized deviation `dev_j = (c_j − d_j)/scale_j`, **per-severity** scale `[D_max^{sev_k}, 1, AoI_max^{sev_k}, 1, R_REF]`-blocked (ĐX2 — `build_dual_scales(K, severity_per_amb)` rebuilt each episode, `lagrangian.py:218,252`; C1/C4 scale tracks each ambulance's own `D_max^{sev_k}`/`AoI_max^{sev_k}`, C2/C5 = 1 probability, C3 = R_REF). The fixed `[D_REF, 1, AoI_REF, 1, R_REF]` form is the **init-only fallback** before severity is known (`lagrangian.py:170`). Threshold subtracted **exactly once**.
...
`_feasible_rrm_cap` (`oran_env.py:652`) reserves `⌈10·1e6 / C_PRB@0dB⌉ = 38` PRB for eMBB at the conservative 0 dB rate and clips the Manager budget. Consequence: **C3 is structurally satisfied at SINR ≥ 0 dB** — at exactly 0 dB, 38 PRB delivers exactly 10 Mbps (the hard floor); at the experimental cell-edge SINR ≈ 2.7 dB, 38 PRB delivers ≈ 16.8 Mbps (margin above floor). Note: 16.8 Mbps is the throughput at a specific operating point, NOT the hard floor — the hard floor is 10 Mbps at 0 dB. Separately, `audit/feasibility_oracle.py` (M/G/1+Shannon, independent of env/RL) finds that after URLLC takes its required share, eMBB receives **97.1 Mbps at worst-case [5,5,5] edge heavy** (9.7× the floor). The learned λ_C3 activates only in **deep fade (SINR < 0 dB)**, where the 0 dB-conservative reservation under-delivers; residual C3 violation is still measured from realized throughput. With floor 10 Mbps the effective ceiling stays `B_RRM_MAX=0.85`.
