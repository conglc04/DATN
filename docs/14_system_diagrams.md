...
Non-RT RIC (Manager ~1s) → Near-RT RIC (Worker 10ms) → O-DU/O-RU MAC (TTI 0.5ms); W=10 Worker/Manager. Single-cell UMi R=300m @ Bạch Mai. Xem [03_architecture](03_architecture.md).
...
## 5. Không gian quan sát (31-dim, K=1) & hành động (6-dim)
![State/Action](figures/05_state_action.svg)

obs = 20 fixed + 10K + F(=1) = 31 (K=1; K=3,F=1 → 51); severity_ref one-hot tại [10:15], λ_local_C3_shared (scalar) tại [15]; per-amb 10-dim block (`SINR_k,d_k,v_k,delay_norm_k,AoI_norm_k,severity_norm_k,λ_C1_k,λ_C2_k,λ_C4_k,λ_C5_k`) (2026-06-15 severity_per_amb epic: per-xe `severity_per_amb[k]∈{1..5}` độc lập/cố định-episode, `severity_ref:=max` cho shared; obs 30→31, formula `24+5K+F`→`20+10K+F`; lịch sử trước: phase→severity swap 33→31, F=4→F=1 31→28, per-ambulance queue/AoI split 28→30). Action 6-dim (K=1, β≡BETA_MIN cố định) → RRMPolicyRatio [TS 28.541]; K≥2: action 7-dim (+`a_beta`→β∈[BETA_MIN,BETA_MAX]=[0,5]). Xem [07_api_spec](07_api_spec.md).
...
## 7. Một vòng chạy hoàn chỉnh — 1 episode (1 s)
![One episode](figures/07_one_episode.svg)

Phân cấp 3 thang thời gian lồng nhau cho **một episode**:
**Episode** (reset → sample severity cố định + load `λ_warm[sev]`) → **Manager step ×10** (rApp ~100ms: `on_manager_step_start(sev)` no-op vì severity cố định + `manager.act → b_rrm`) → **Worker step ×W=10** (xApp 10ms: overlay λ → `worker.act` → `env.step`) → **20 MAC tick** (O-DU 0.5ms). Cuối mỗi Manager step: **dual ascent** `λ←clip(λ+α_λ·ĝ,0,10)` với `ĝ=win_c/win_steps`. Cuối episode: PPO update (Worker+Manager buf, GAE) / TD3-SAC đã update mỗi step; flush `λ_warm[sev]`.
Tổng: `10 × 10 × 20 = 2000 TTI × 0.5ms = 1.0 s`. Xem [13 §1.4](13_methodology_walkthrough.md) (thang thời gian) + `train.py`.
