...
- **`_prb_split_intra_slice(prb_urllc)`**: `b=max(floor(INTRA_SLICE_KAPPA·B_U/K), PRB_MIN_QOS)` (fallback `b=B_U//K` nếu `K·b>B_U`), `S=B_U-K·b`, `w=softmax(β·severity_per_amb + δ·ũ)` (`δ=RHO_URGENCY_TIEBREAK·β`, `ũ_k=λ_C1_k/max(λ_C1)`), `PRB_k=b+S·w_k`. K=1: `softmax([x])=[1.0]` ∀x ⟹ `PRB_0=B_U` luôn (numeric preservation, không phụ thuộc β/severity/urgency).
