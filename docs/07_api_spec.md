...
- **K=1 = 1-dim**: no-op scalar (xe duy nhất nhận toàn bộ URLLC PRBs, β≡BETA_MIN cố định). Bounds `[-3]→[3]`.
- **K≥2 = (1+K)-dim**: `a[0]`→β priority temperature (`β = BETA_MIN + (BETA_MAX−BETA_MIN)·sigmoid(a[0])`, `BETA_MIN=0.5, BETA_MAX=5.0`); `a[1:1+K]`→per-vehicle priority logits w_k (raw ∈[−3,+3], decode qua softmax trong `_prb_split_intra_slice`). K=3 → 4-dim.
- **Worker KHÔNG điều khiển inter-slice**: `r_min/r_max/r_ded` trong obs[4:7] là Manager-owned (read-only cho Worker). `{Δr_min, Δr_max, r_ded, w_C1, w_C2, w_C3}` 6-dim = **legacy, ĐÃ GỠ** — Worker action hiện chỉ gồm β + per-vehicle logits.
- `β_min/β_max` + anti-starvation floor `PRB_MIN_QOS=1` = hyperparam (KHÔNG action). `urgency_k` DERIVED từ λ_C1 trong obs (KHÔNG thêm field).

## Mapping → 3GPP RRMPolicyRatio (✅[TS 28.541])

Manager `b_rrm`→`RRMPolicyMinRatio` (URLLC fraction); `1−b_rrm`→`RRMPolicyMaxRatio` (eMBB remainder). Worker per-vehicle logits → intra-slice Π_feasible split (không mapping trực tiếp 3GPP — đây là scheduler-internal).
