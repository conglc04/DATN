...
- `baselines/smoke_train.py` — smoke cho td3_lag + b3.
- `tests/test_train_loop.py` — 5-ep smoke.

## Gate G3.2 ✅
- `train.py --algo pa_chrl_ppo --episodes 5` chạy không crash; `metrics.csv` 5 rows finite (no NaN/Inf); λ_global đôi khi >0 (constraint binding visible); baseline smoke (b3, td3_lag) chạy với 5-dim λ.
