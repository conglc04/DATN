...
> **REVISED 2026-06-13**: W18–W23 = sweep tuần tự **3 solver × K∈{1,3}** (PPO K=1 → PPO K=3 → TD3 K=1 → TD3 K=3 → SAC K=1 → SAC K=3), env config khóa: gNB/cell-center=`(0,0)`, R_cell=1km, UMa single-cell + interference margin −86 dBm/PRB, no handover.
...
|---|---|---|---|
| [W12](W12_pha1_radio_channel_capacity.md) | 1 | Radio + channel UMa 1km Bạch Mai + capacity (M1–M3) | 1A |
...
- **Env config (XUYÊN SUỐT)**: gNB/cell-center=`(0,0)` local Cartesian, R_cell=1km, UMa (3GPP TR 38.901 `pl_uma`) single-cell + interference margin −86 dBm/PRB, no handover. GPS thật BV Bạch Mai (`config.py: BACH_MAI_LAT/LON`) chỉ dùng cho lớp OSM/SUMO (W15), KHÔNG dùng trực tiếp trong RL env.
