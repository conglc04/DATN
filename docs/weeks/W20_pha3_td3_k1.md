...
gNB/cell-center = `(0,0)`, R_cell=1km, UMa 3GPP TR 38.901 + interference margin −86 dBm/PRB, single-cell, no handover. `K_ambulances=1` → obs=31 (như W18).
...
- **A-TD3.2** Train `TD3Solver` trên env K=1, ≥10 seeds, replay buffer + warmup (500 steps) như cấu hình mặc định `td3.py`.
