...
> **Pha**: 1 · **Status**: ✅ DONE · **Gate**: micro-GATE 1B ✅ · **Nhóm**: M4–M6 · **Deps**: W12/1A ✅

## M4 — Phân rã độ trễ `D_e2e = D_DET + d_tx + d_queue + D_FH + D_BH`
- **M4.1** `D_DET = 0.07 ms` (xử lý N1+N2) — ✅[3GPP TS 38.214 §5.3 / TS 38.213 UE processing 3–4 OFDM symbols ≈ 0.04–0.14 ms @μ=1; midpoint 0.07 ms] — [ms]
- **M4.2** `d_tx, d_queue` — hàng đợi **M/G/1 Pollaczek–Khinchine** — ✅[`9780470316887.pdf` (Kleinrock) §5.6] — [ms]
- **M4.3** `D_FH = 0.1 ms` (fronthaul) — 🔴 declared [O-RAN.WG4.CTR v03.00 Table 4-2, ngoài corpus → ASSUMPTION; REFERENCE_MAP §2]
- **M4.4** `D_BH = 0.1 ms` (backhaul) — ✅[3GPP TR 38.801 §6.3 transport budget; 0.1 ms = engineering lower-bound Metro MEC topology; ASSUMPTION — REFERENCE_MAP §2]
...
- **M6.2** `D_max^{sev4} = 2 ms` (EMERGENCY) + các mức khác (sev1 NON_URGENT 20ms → sev5 IMMEDIATE 1ms, bảng [02](02_requirements.md)) — ✅[3GPP TS 22.261 Annex A Table A.1-1; 5×4 table pinned in `test_formulas_config_tables_exact.py` + monotonicity test]
...
Giải `PRB_k` nhỏ nhất sao cho `D_e2e(PRB_k) ≤ D_max^{sev_k}`, với `C_k(PRB_k)=η·PRB_k·B_PRB·log₂(1+SINR_cell-edge)` (M3.1 @SINR cell-edge ≈ UMa@1km +2.7dB) và `d_queue` từ M/G/1 PK (M4.2) tại tải M8. `D_e2e` đơn điệu giảm theo PRB_k ⟹ nghiệm duy nhất. 🟡 (form đã rõ; numeric solve khi M3/M6/M8 khóa). **KHÔNG** claim "guarantee 3GPP ∀ channel" — chỉ sàn PRB-allocation @SINR_ref (audit #6).
