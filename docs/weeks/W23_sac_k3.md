...
`test_multi_ambulance` + `test_intra_slice` pass với K=3 cho SAC; λ-trajectory + saturation-rate đã log cho cả 6 λ (C1-C5+C6); mỗi p-value kèm test-name + n_seeds + hiệu chỉnh + Hedges' g + bootstrap CI; Table II KHÔNG Jain toàn cục.

## GATE 3F
SAC K=3 train hội tụ trên obs=58; 2 structural guarantee xác nhận cho SAC; Table I (6 cell × 3 solver) + Table II (severity/intra-slice, K=3) hoàn tất với CI + p-value hiệu chỉnh; KHÔNG báo cáo "thắng" nếu CI chồng lấn.
