...
- **A-SAC.1** Train `SACBaseline` trên env K=1, ≥10 seeds, replay buffer + warmup (500 steps) như cấu hình mặc định `sac.py`.
...
## ⟲ RÀ SOÁT SAC K=1
`test_oran_env_sanity` + `test_reward_constraint_tracking` pass với obs=33 (env không đổi — chỉ solver mới SAC); SAC code mirror đúng convention TD3 (cùng `BaselineFlags`, cùng `LambdaState`); KHÔNG so sánh chéo solver trong tuần này (để dành Table I/II ở W24).
