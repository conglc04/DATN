...
| W18 | 3 | B0/B0b+B5 | Code K≥2 (assert obs 32/54; removals) + **PPO K=1** | **3A** |
...
gNB/cell-center=`(0,0)` (local Cartesian, điểm hội tụ 3 xe trên Giải Phóng), R_cell=1km, UMa 3GPP TR 38.901 single-cell + interference margin −86 dBm/PRB, no handover. GPS thật BV Bạch Mai (`config.py: BACH_MAI_LAT/LON`) chỉ dùng cho lớp OSM/SUMO (W15). Chỉ `K_ambulances` (1↔3) và solver (PPO/TD3/SAC) thay đổi theo tuần.
